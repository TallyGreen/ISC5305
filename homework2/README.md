...
```
But it nom function doesn't work. I aks ChatGPT to help me fix this problem.
...
int main()
{
    ...
    Particle particle2D(1.0, Vector(0.0, 0.0), Vector(1.0, 2.0));  // 2D

    // Create a 3D particle with mass 1
    Particle particle3D(1.0, Vector(0.0, 0.0, 0.0), Vector(1.0, 2.0, 3.0));  // 3D
...
```
But I found result is not correct when I used test.cpp. I used ChatGPT to figure it out. The problem in the main(). I change main() part as:
```cpp
int main()
{
    try {
