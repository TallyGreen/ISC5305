...
double norm(const Vector &v, const string type)
{
    ...
    if (type == "L1") {
        double sum = 0.0;
        for (int i = 0; i < v.size(); ++i) {
            sum += abs(v[i]);  // Summing the absolute values of the components
        }
        return sum;
    }
    // L2 norm: square root of the sum of the squares of the components
        ...
        for (int i = 0; i < v.size(); ++i) {
            sumSquares += v[i] * v[i];  // Summing the squares of the components
        }
        return sqrt(sumSquares);  // Taking the square root of the sum
    }
    // Linf norm: the maximum absolute value of the components
    else if (type == "Linf") {
        double maxVal = 0.0;
        for (int i = 0; i < v.size(); ++i) {
            maxVal = max(maxVal, abs(v[i]));  // Finding the maximum absolute value
        }
        return maxVal;
    }
